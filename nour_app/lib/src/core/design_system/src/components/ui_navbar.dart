...
/// Lightweight prayer-beads (tasbih) glyph painted to match the design,
/// avoiding the need for a dedicated SVG asset.
class _TasbihPainter extends CustomPainter {
  const _TasbihPainter({required this.color});
  final Color color;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final loopCenter = Offset(size.width / 2, size.height * 0.42);
    final loopRadius = size.width * 0.34;
    ...
    for (var i = 0; i < beadCount; i++) {
      ...
      final c = Offset(
        loopCenter.dx + loopRadius * math.cos(angle),
        loopCenter.dy + loopRadius * math.sin(angle),
      );
      canvas.drawCircle(c, beadRadius, paint);
    }

    // Tassel hanging from the bottom bead.
    final topTassel = Offset(loopCenter.dx, loopCenter.dy + loopRadius);
    final endTassel = Offset(size.width / 2, size.height * 0.96);
    canvas.drawLine(topTassel, endTassel, stroke);
    canvas.drawCircle(endTassel, beadRadius * 1.2, paint);
  }

  @override
  bool shouldRepaint(_TasbihPainter oldDelegate) => oldDelegate.color != color;
}
