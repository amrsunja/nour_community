...
import 'favorite_hadiths_state.dart';

/// App-lifetime provider (NOT autoDispose): see [favoriteAyahsProvider].
final favoriteHadithsProvider =
    StateNotifierProvider<FavoriteHadithsPresenter, FavoriteHadithsState>((ref) {
  return FavoriteHadithsPresenter(
    repo: ref.read(favoritesRepoProvider),
    appEvents: ref.read(appEventProvider),
  );
});

class FavoriteHadithsPresenter extends Presenter<FavoriteHadithsState> {
  final FavoritesRepo repo;
  final AppEvents appEvents;

  FavoriteHadithsPresenter({
    required this.repo,
    required this.appEvents,
  }) : super(const FavoriteHadithsState());
  ...
  Future<void> _load({bool silent = false}) async {
    ...
    final res = await repo.getFavoriteHadiths();
    res.when(
      (items) =>
