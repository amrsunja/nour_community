...
- Stripe Dashboard → Developers → Webhooks → **"Listen to events on Connected accounts"** → endpoint `https://<project>.supabase.co/functions/v1/stripe-connect-webhook`, events: `account.updated`, `payment_intent.succeeded`, `payment_intent.payment_failed`, `payment_intent.canceled`, `invoice.paid`, `invoice.payment_succeeded`, `invoice.payment_failed`, `customer.subscription.updated`, `customer.subscription.deleted`. Copy the signing secret into `STRIPE_CONNECT_WEBHOOK_SECRET`. **The invoice events are what book a recurring gift into the ledger** — without them a subscription is paid but adds nothing to the mosque totals or the campaign progress (`confirm-mosque-payment` then repairs it the next time the donor opens the app). The existing platform webhook stays untouched.
